...
const calculateOrderTotal = (order) => {
  ...
  for (const item of order) {
    if (item.quantity > item.available) {
      throw new Error(
        `Cannot fulfill order: the amount requested (${item.quantity}) is greater than the amount available (${item.available})`
      );
    }
...
const handler: Handler = async (req) => {
  try {
    ...
    if (validateQuantity(items, books)) {
      ...
      const shippingRate = await new Promise<number>((resolve, reject) => {
        try {
